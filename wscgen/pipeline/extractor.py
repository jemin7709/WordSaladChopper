...
class HiddenStateExtractor:
    def __init__(self, model_name: str,
        ...
        self.model = (AutoModelForCausalLM
                      .from_pretrained(model_name, torch_dtype=torch.bfloat16, device_map="auto")
                      .eval())
        self.device = device
    ...
    def extract_token_states(
        ...
        """
        Return (token_texts, states) where `states` shape = [T, D].
        Only the layer `self.layer_idx` is returned.
        """
        ...
        h16 = out.hidden_states[self.layer_idx][0]
        bad = (~torch.isfinite(h16)).sum().item()
        logger.info(f"[overflow check] non-finite: {bad}/{h16.numel()}")

        h = out.hidden_states[self.layer_idx].squeeze(0)
        h = h.detach().cpu().numpy()

        if not include_special:
            token_ids = tok_out.input_ids[0].cpu().tolist()
            token_texts = self.tokenizer.convert_ids_to_tokens(token_ids)
        else:
            token_texts = [
                self.tokenizer.decode([tid]) for tid in tok_out.input_ids[0]
            ]
    ...
    def extract_sentence_states(self,
        ...
        if self.module_path is None:
            ...
            h = outs.hidden_states[self.layer_idx][0]        # [seq, dim]
        else:
            self._last_hidden = None
            with torch.no_grad():
                _ = self.model(**inputs)
