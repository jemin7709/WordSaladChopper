...
def load_config(cfg_path: Optional[Union[str, Path]] = None) -> dict:
    ...
    Parameters
    ----------
    cfg_path : Optional[Union[str, Path]]
        • Path / str  →  load that file  
        • None        →  fallback to package default
                         (<package_root>/configs/tiny.yaml)
    ...
    # 1. parse path
    if cfg_path is None:
        cfg_path = pkg_rs.files("wscgen").joinpath("configs", "tiny.yaml")
    else:
