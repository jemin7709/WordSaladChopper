...
class SentenceLabeler:
    def __init__(
        ...
        if self.labeler_type == "semantic":
            ...
            logger.info(
                f"[SentenceLabeler] semantic mode – loading '{embed_model}' on {device}"
            )
    ...
    def label_texts(
        ...
        # 2) embed all chunks in one shot
        embs = (
            self.embedder.encode(
                all_chunks,
                batch_size=bs_embed,
        ...
        for s_idx, chunks in enumerate(sample_chunks):
            ...
            for i in range(n_chunks):
                ...
                sim = torch.matmul(curr_emb, prev_embs.T)[0]  # shape [i-j0]
                sim_scores[i] = sim.max().item() if sim.numel() > 0 else 0.0

            # --- find change‑point ---
            boundary = None
            if n_chunks >= L:
                # sliding window over similarity scores to find first streak of length L ≥ th
                for i in range(L - 1, n_chunks):  # i is the right‑most idx of the streak
    ...
    def _label_rule(self, texts: Sequence[str]) -> List[List[Tuple[str, int, float]]]:
        """
        For rule mode, use exact-match to track duplicates
        """
        results = []
        for txt in texts:
            seen = set()
...
def build_labeler(cfg: PipelineConfig, tokenizer: AutoTokenizer | None = None):
    """Instantiate the correct labeler from the experiment config."""
    lp = cfg.labeler_params or {}
    return SentenceLabeler(
        labeler_type=cfg.labeler_type,
        sim_threshold=lp.get("sim_threshold", 0.9),
        embed_model=lp.get("embed_model", "sentence-transformers/all-mpnet-base-v2"),
        device=lp.get("device", "auto"),
        tokenizer=tokenizer,
        slide_window=lp.get("slide_window", 100),
